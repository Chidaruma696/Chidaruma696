...
| 🐉 | [**Kobayashi**](https://github.com/Chidaruma696/Kobayashi) | Point of sale and back office for a meat plant that labels by weight: orders, production with waste, EAN-13 identity labels with a Web Serial scale, dispatch with double scan, receiving at the shop, register with cash close, counts with charges, delivery trips with the driver's stop on the phone, route credit by type, collections, crates and price agreements; whatever is done without authorisation goes to a review tray instead of stopping the flow | Ruby on Rails · Hotwire · SQLite |
